            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="content-area">
                        ...
                        @endif
                        @if(! $books->count())
                            <div class="alert alert-warning my-4">Kitob mavjud emas</div>
                        @else
                            <div class="card my-4">
                                <div class="card-header bg-dark">
                                    <h4 class="text-white">Hamma kitoblar</h4>
                                </div>
                                @if(isset($categoryName))
                                    <div class="alert alert-info m-3">
                                        Kitoblar - <strong>{{$categoryName}}</strong>
                                    </div>
                                @endif
                                @if(isset($authorName))
                                    <div class="alert alert-info m-3">
                                        Yozuvchi - <strong>{{$authorName}}</strong>
                                    </div>
                                @endif
                                                ...
                                                <div class="book-wrap">
                                                    <div class="book-image mb-2">
                                                        ...
                                                        @if($book->discount_rate)
                                                            <h6><span class="badge badge-warning discount-tag">Chegirma {{$book->discount_rate}}%</span></h6>
                                                        @endif
                                                    </div>
                                                    ...
                                                    <div class="book-author mb-2">
                                                        <small>Avtor <a href="{{route('author', $book->author->slug)}}">{{$book->author->name}}</a></small>
                                                    </div>
